...
export const update = async (req, res) => {
   try {
      ...
      //    const oldProduct = await ProductModel.findById(req.params.productId);
      //    const oldProductImgs = oldProduct._doc.imgNames;
      //    const newProductImgs = req.body.imgNames;
      //    const foldername = "static/" + req.params.productId;

      //    for (let tmp of oldProductImgs) {
      //       if (!newProductImgs.includes(tmp)) {
      //          await deleteFile(tmp);
      //       }
      //    }

      //    let index;
      //     getFilesInFolder("static" + req.params.productId).then(
      //       (files) => {
      //          map = files.map((x) => x.split(".")[0]);
      //          index = Math.max(...map);
      //       }
      //    );

      //    for (let tmp of newProductImgs) {
      //       if (!oldProductImgs.includes(tmp)) {
      //          index = index + 1;
      //          await moveFile(
      //             "static/" + tmp,
      //             foldername + "/" + index + "." + tmp.split(".")[1]
      //          );
      //       }
      //    }
...
export const create = async (req, res) => {
   try {
      ...
      // if (!fs.existsSync("static/" + product._id)) {
      //    fs.mkdirSync("static/" + product._id);
      // }

      // if (req.body.imgNames) {
      //    const foldername = "static/" + product._id;
      //    let files = [];
      //    await req.body.imgNames.map(async (element, i) => {
      //       await moveFile(
      //          "static/" + element,
      //          foldername + "/" + i + "." + element.split(".")[1]
      //       );
      //       files.push(foldername + "/" + i + "." + element.split(".")[1]);
      //    });
